...
def upgrade():
    ...
    op.create_foreign_key(
        None, "tasks", "users", ["task_author_id"], ["id"], ondelete="CASCADE"
    )
    op.add_column("users", sa.Column("github_id", sa.BigInteger(), nullable=True))
    op.add_column(
        "users", sa.Column("github_avatar_url", sa.Unicode(length=500), nullable=True)
    )
    op.add_column(
        "users", sa.Column("github_access_token", sa.Unicode(length=500), nullable=True)
    )
...
def downgrade():
    ...
    op.create_foreign_key(
        "tasks_task_author_id_fkey", "tasks", "users", ["task_author_id"], ["id"]
    )
    op.create_index("idx_events_timestamp", "events", ["timestamp"], unique=False)
    op.create_index(
        "idx_events_team_member_id", "events", ["team_member_id"], unique=False
    )
    # ### end Alembic commands ###
