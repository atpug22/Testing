...
from app.models import TeamMember, PullRequest, Event, User
from app.repositories import TeamMemberRepository, EventRepository, PullRequestRepository
...
class TeamMemberController(BaseController[TeamMember]):
    ...
    def calculate_primary_status(
        ...
        blocked_prs = [pr for pr in recent_prs if pr.flow_blockers and len(pr.flow_blockers) > 0]
        ...
        if recent_prs:
            bug_prs = [pr for pr in recent_prs if pr.labels and 'bug' in [l.lower() for l in pr.labels]]
        ...
        return {
            ...
            "in_discussion": {
                ...
                "hover_details": [
                    ...
                    for pr in wip_prs if pr.unresolved_comments > 0
                ][:3]
            },
            "last_active": {
                "label": "Last Active",
    ...
    def generate_copilot_insights(
        ...
        merged_last_week = [
            ...
            if pr.merged_at and make_aware(pr.merged_at) > datetime.now(timezone.utc) - timedelta(days=7)
        ]
        ...
        stale_prs = [
            pr for pr in recent_prs
            if pr.status == "open" and make_aware(pr.created_at) < datetime.now(timezone.utc) - timedelta(days=7)
        ]
        if len(stale_prs) >= 2:
            insights.append({
                "type": "risk",
        ...
        if team_member.review_velocity_median_hours and team_member.review_velocity_median_hours > 48:
            insights.append({
                "type": "health",
    ...
    async def compute_work_focus_metrics(
        self, user_id: int
    ) -> Dict[str, Any]:
