...
from app.models.enums import FlowBlocker, PrimaryStatus, RiskFlag
...
class GitHubFetcher:
    ...
    async def fetch_repository_data(
        ...
        for i, pr_data in enumerate(detailed_prs):
            try:
                ...
                try:
                    existing_pr = await pr_repo.get_by(
                        "github_pr_id", pr_data["number"], unique=True
                    )
                except Exception:
                    existing_pr = None
                ...
                if existing_pr:
                    if i < 5:  # Only show detailed debug for first 5 PRs
                        print(
                            f"    ⚠️  PR #{pr_data.get('number', 'unknown')} already exists, updating..."
                        )
                    # Update existing PR
                    ...
                    existing_pr.unresolved_comments = pr_data.get(
                        "comments", 0
                    ) + pr_data.get("review_comments", 0)
                    existing_pr.lines_changed = pr_data.get(
                        "additions", 0
                    ...
                    if i < 5:  # Only show detailed debug for first 5 PRs
                        print(
                            f"    ✅ PR #{pr_data.get('number', 'unknown')} updated successfully"
                        )
                else:
                    ...
                    pr = PullRequest(
                        ...
                        unresolved_comments=pr_data.get("comments", 0)
                        + pr_data.get("review_comments", 0),
                        lines_changed=pr_data.get("additions", 0)
                        + pr_data.get("deletions", 0),
                    ...
                    if i < 5:  # Only show detailed debug for first 5 PRs
                        print(
                            f"    ✅ PR #{pr_data.get('number', 'unknown')} stored successfully"
                        )
            except Exception as e:
                print(
                    f"  ⚠️  Error processing PR #{pr_data.get('number', 'unknown')}: {e}"
                )
    ...
    async def _get_or_create_user(self, user_data: Dict[str, Any]) -> User:
        ...
        if user_data.get("id"):
            try:
                existing_user = await user_repo.get_by(
                    "github_id", user_data["id"], unique=True
                )
                if existing_user:
        ...
        if user_data.get("login"):
            try:
                existing_user = await user_repo.get_by(
                    "username", user_data["login"], unique=True
                )
                if existing_user:
    ...
    def _analyze_flow_blockers(self, pr_data: Dict[str, Any]) -> List[str]:
        ...
        if pr_data.get("changed_files", 0) > 5 and not any(
            "test" in label.lower() for label in pr_data.get("labels", [])
        ):
    ...
    def _analyze_risk_flags(self, pr_data: Dict[str, Any]) -> List[str]:
        ...
        if any(
            critical in pr_data.get("title", "").lower() for critical in critical_files
        ):
    ...
    async def _create_timeline_events(
        self, prs: List[PullRequest], owner: str, repo: str
    ):
        ...
        for pr in prs:
            ...
            event = Event(
                team_member_id=(
                    pr.author.team_member_profile.id
                    if pr.author.team_member_profile
                    else None
                ),
        ...
        for author_id, author_pr_list in author_prs.items():
            ...
            if total_prs > 0:
                feature_prs = len(
                    [
                        pr
                        for pr in author_pr_list
                        if any("feat" in label.lower() for label in (pr.labels or []))
                    ]
                )
                bug_prs = len(
                    [
                        pr
                        for pr in author_pr_list
                        if any("bug" in label.lower() for label in (pr.labels or []))
                    ]
                )
                chore_prs = len(
                    [
                        pr
                        for pr in author_pr_list
                        if any("chore" in label.lower() for label in (pr.labels or []))
                    ]
                )
            ...
            if team_member.wip_count > 5:
                ...
                team_member.wip_count == 0 and team_member.merged_prs_last_30_days > 10
            ):
