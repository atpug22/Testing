...
async def get_organizations(
    ...
    organization_controller: OrganizationController = Depends(
        Factory().get_organization_controller
    ),
...
@organization_router.get(
    "/{organization_id}", dependencies=[Depends(AuthenticationRequired)]
)
...
async def update_organization(
    ...
    if organization.owner_id != user.id:
        raise HTTPException(
            status_code=403, detail="Only organization admin can update"
        )
    ...
    updated_org = await organization_controller.update(
        id_=organization_id, obj=organization
    )
...
async def delete_organization(
    ...
    if organization.owner_id != user.id:
        raise HTTPException(
            status_code=403, detail="Only organization owner can delete"
        )
...
@organization_router.post(
    "/{organization_id}/invite", dependencies=[Depends(AuthenticationRequired)]
)
async def invite_member(
    ...
    invitation_controller: InvitationController = Depends(
        Factory().get_invitation_controller
    ),
) -> InvitationResponse:
    ...
    if role != OrganizationRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Only organization admins can invite members"
        )
    ...
    pending_invitations = await invitation_controller.get_pending_by_email(
        request.email
    )
...
@organization_router.get(
    "/{organization_id}/invitations", dependencies=[Depends(AuthenticationRequired)]
)
async def get_organization_invitations(
    ...
    if role != OrganizationRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Only organization admins can view invitations"
        )
