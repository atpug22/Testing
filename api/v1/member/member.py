...
async def get_member_insights(
    ...
    return CopilotInsightsResponse(
        insights=insights, generated_at=datetime.now(timezone.utc)
    )
...
async def get_member_metrics(
    ...
    velocity = await team_member_controller.compute_velocity_metrics(
        team_member.user_id
    )
    work_focus = await team_member_controller.compute_work_focus_metrics(
        team_member.user_id
    )
    quality = await team_member_controller.compute_quality_metrics(
        team_member.user_id, team_member
    )
    collaboration = await team_member_controller.compute_collaboration_metrics(
        team_member
    )
