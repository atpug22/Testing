...
class AIAuthorshipDetector:
    ...
    def analyze_pr(self, pr_data: Dict[str, Any]) -> AIAuthorshipResult:
        ...
        if "files" in pr_data and pr_data["files"]:
            file_score, file_indicators, file_details = self._analyze_files(
                pr_data["files"]
            )
            ai_score += file_score
        ...
        if "commits" in pr_data and pr_data["commits"]:
            commit_score, commit_indicators, commit_details = self._analyze_commits(
                pr_data["commits"]
            )
    ...
    def _analyze_files(
        ...
        for file_data in files:
            ...
            if patch:
                ...
                if ai_matches:
                    ...
                    indicators.extend(
                        [f"AI pattern in {filename}: {match}" for match in ai_matches]
                    )
        ...
        if len(details["file_types"]) >= self.ai_file_patterns["multiple_file_types"]:
            score += 0.4
            indicators.append(
                f"Multiple file types modified: {list(details['file_types'])}"
            )
    ...
    def _determine_confidence(
        self, ai_probability: float, indicator_count: int
    ) -> AIConfidenceLevel:
    ...
    def batch_analyze_prs(
        self, prs_data: List[Dict[str, Any]]
    ) -> List[AIAuthorshipResult]:
