...
from ai_impact_models import (
    AIImpactAnalysis, AIImpactMetrics, AITrendAnalysis, 
    AIQualityAssessment, AIConfidenceLevel, AIAuthorshipResult
)
...
class AIImpactAnalyzer:
    ...
    def __init__(self, storage_dir: Optional[str] = None):
        ...
        self.storage_dir = Path(storage_dir) if storage_dir else Path(__file__).parent / "ai_impact_data"
        self.storage_dir.mkdir(parents=True, exist_ok=True)
    
    def analyze_repository(self, repo_data: Dict[str, Any], days: int = 90) -> AIImpactAnalysis:
    ...
    def _calculate_metrics(self, prs: List[Dict[str, Any]], analyses: List[AIAuthorshipResult]) -> AIImpactMetrics:
        ...
        for pr, analysis in zip(prs, analyses):
            if analysis.confidence in [AIConfidenceLevel.HIGH, AIConfidenceLevel.MEDIUM]:
                ai_prs.append(pr)
    ...
    def _calculate_pr_performance(self, prs: List[Dict[str, Any]]) -> Dict[str, Optional[float]]:
        ...
        return {
            ...
            'avg_files_changed': sum(files_changed) / len(files_changed) if files_changed else None,
            'avg_lines_changed': sum(lines_changed) / len(lines_changed) if lines_changed else None
        }
    
    def _analyze_trends(self, prs: List[Dict[str, Any]], analyses: List[AIAuthorshipResult], days: int) -> AITrendAnalysis:
        ...
        for pr, analysis in zip(prs, analyses):
            ...
            if (analysis.confidence in [AIConfidenceLevel.HIGH, AIConfidenceLevel.MEDIUM] or
                (analysis.confidence == AIConfidenceLevel.LOW and analysis.ai_probability >= 0.3)):
        ...
        for week, data in weekly_data.items():
            ...
            weekly_ai_adoption[week] = data['ai'] / data['total'] if data['total'] > 0 else 0.0
    ...
    def _assess_quality(self, prs: List[Dict[str, Any]], analyses: List[AIAuthorshipResult]) -> AIQualityAssessment:
        ...
        for pr, analysis in zip(prs, analyses):
            ...
            if analysis.confidence == AIConfidenceLevel.HIGH and len(analysis.indicators) > 5:
                risk_score += 0.1
    ...
    def _calculate_impact_score(self, metrics: AIImpactMetrics, trends: AITrendAnalysis, quality: AIQualityAssessment) -> float:
        ...
        if (metrics.ai_avg_merge_time_hours is not None and 
            metrics.human_avg_merge_time_hours is not None):
            # AI faster = positive, AI slower = negative
            if metrics.human_avg_merge_time_hours > 0:
                time_ratio = metrics.ai_avg_merge_time_hours / metrics.human_avg_merge_time_hours
    ...
    def _determine_overall_confidence(self, analyses: List[AIAuthorshipResult]) -> AIConfidenceLevel:
        ...
        elif medium_ratio >= 0.3 or confidence_counts[AIConfidenceLevel.LOW] / total >= 0.4:
    ...
    def _generate_insights(self, metrics: AIImpactMetrics, trends: AITrendAnalysis, quality: AIQualityAssessment) -> List[str]:
        """Generate summary insights from the analysis"""
        insights = []
        
        # Adoption insights
        if metrics.ai_adoption_rate >= 0.5:
            insights.append(f"High AI adoption: {metrics.ai_adoption_rate:.1%} of PRs are AI-generated")
        elif metrics.ai_adoption_rate >= 0.2:
            insights.append(f"Moderate AI adoption: {metrics.ai_adoption_rate:.1%} of PRs are AI-generated")
        else:
            insights.append(f"Low AI adoption: {metrics.ai_adoption_rate:.1%} of PRs are AI-generated")
        
        # Performance insights
        if (metrics.ai_avg_merge_time_hours and metrics.human_avg_merge_time_hours and
            metrics.ai_avg_merge_time_hours < metrics.human_avg_merge_time_hours * 0.8):
            insights.append("AI-generated PRs merge significantly faster than human PRs")
        elif (metrics.ai_avg_merge_time_hours and metrics.human_avg_merge_time_hours and
              metrics.ai_avg_merge_time_hours > metrics.human_avg_merge_time_hours * 1.2):
        ...
        if quality.high_risk_ai_prs:
            insights.append(f"{len(quality.high_risk_ai_prs)} AI PRs flagged for additional review")
