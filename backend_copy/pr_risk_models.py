...
class StucknessMetrics(BaseModel):
    ...
    time_since_last_activity_hours: float = Field(
    ...
    time_waiting_for_reviewer_hours: float = Field(
        description="Hours waiting for required reviewer"
    )
    ...
    linked_issue_stale_time_hours: float = Field(
        description="Average stale time of linked issues"
    )
...
class BlastRadiusMetrics(BaseModel):
    ...
    downstream_dependencies: int = Field(
        description="Number of downstream dependencies"
    )
    critical_path_touched: bool = Field(
        description="Whether critical paths are modified"
    )
    ...
    historical_regression_risk: float = Field(
        description="Similarity to past buggy changes (0-1)"
    )
...
class DynamicsMetrics(BaseModel):
    ...
    author_experience_score: float = Field(
        description="Author experience in repo (0-100)"
    )
...
class BusinessImpactMetrics(BaseModel):
    ...
    linked_to_release: bool = Field(
        description="Whether PR is linked to release milestone"
    )
    external_dependencies: int = Field(
        description="Number of external API/service dependencies"
    )
    priority_label: Optional[str] = Field(description="Priority label from PR")
    affects_core_functionality: bool = Field(
        description="Whether it affects core features"
    )
...
class PRDetailedInfo(BaseModel):
    ...
    # Commits
    commit_count: int = 0
    commits_authors: List[str] = Field(
